test_that("basic clipping", {
  ...
  out <- clip_lines_impl(x, y, id, .5, .5, .5, 2, 0, 1)
...
test_that("clip multiple line segments", {
  ...
  out <- clip_lines_impl(x, y, id, 0, 0, 1, 3, 0, 1)
  ...
  out <- clip_lines_impl(x, y, id, 1.5, 1.5, 2, 4, 0, 1)
...
test_that("singletons are carried over or clipped", {
  ...
  out <- clip_lines_impl(x, y, id, 10, 10, 1, 1, 0, 1)
  expect_identical(out$x, x)
  expect_identical(out$y, y)
  expect_identical(out$id, id)
  ...
  out <- clip_lines_impl(x, y, id, 1, 0, .1, .1, 0, 1)
  ...
  out <- clip_lines_impl(x, y, id, 3, 2, .1, .1, 0, 1)
...
test_that("empty or incorrect input", {
  out <- clip_lines_impl(numeric(0), numeric(0), integer(0), 3, 2, .1, .1, 0, 1)
  expect_identical(out$x, numeric(0))
  expect_identical(out$y, numeric(0))
  expect_identical(out$id, integer(0))

  expect_snapshot(
    clip_lines_impl(numeric(0), numeric(1), integer(0), 3, 2, .1, .1, 0, 1),
    error = TRUE
  )
  expect_snapshot(
    clip_lines_impl(numeric(0), numeric(0), integer(1), 3, 2, .1, .1, 0, 1),
    error = TRUE
  )
  expect_snapshot(
    clip_lines_impl(numeric(1), numeric(0), integer(0), 3, 2, .1, .1, 0, 1),
    error = TRUE
  )
})
