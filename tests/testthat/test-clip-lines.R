...
test_that("rotated clip box", {
  ...
  out <- clip_lines(x, y, id, c(.5, .5), 5, sin(2*pi*45/360), 2*pi*45/360)
  ...
  out <- clip_lines(x, y, id, c(.5, .5), sin(2*pi*45/360), 5, 2*pi*45/360)
  expect_equal(out$x, c(0.0, 0.0, 0.5, 1.0, 1.0, 0.5, 0.0))
  expect_equal(out$y, c(0.0, 0.5, 1, 1, 0.5, 0, 0))
  expect_identical(out$id, c(rep(1L, 2), rep(2L, 3), rep(3L, 2)))
})
...
test_that("singletons are carried over or clipped", {
  ...
  out <- clip_lines(x, y, id, c(10, 10), 1, 1, 0)
  expect_identical(out$x, x)
  expect_identical(out$y, y)
  expect_identical(out$id, id)
  ...
  out <- clip_lines(x, y, id, c(1, 0), .1, .1, 0)
  ...
  out <- clip_lines(x, y, id, c(3, 2), .1, .1, 0)
...
test_that("empty or incorrect input", {
  out <- clip_lines(numeric(0), numeric(0), integer(0), c(3, 2), .1, .1, 0)
  expect_identical(out$x, numeric(0))
  expect_identical(out$y, numeric(0))
  expect_identical(out$id, integer(0))

  expect_error(
    clip_lines(numeric(0), numeric(1), integer(0), c(3, 2), .1, .1, 0),
    "must match"
  )
  expect_error(
    clip_lines(numeric(0), numeric(0), integer(1), c(3, 2), .1, .1, 0),
    "must match"
  )
  expect_error(
